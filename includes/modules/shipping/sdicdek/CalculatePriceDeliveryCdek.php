<?php
/**
 * CalculatePriceDeliveryCdek.php  19.11.20 13:54
...
class CalculatePriceDeliveryCdek {
	...
	public function calculate() {
		...
		$response = $this->_getRemoteData($data,$url);
	...
	public function getCityIdByName($cityName){
		...
		$cityName = trim(preg_replace("/^г /si", "", $cityName, 1));

//		$data = array(
//			"q"=> $cityName,
//			"name_startsWith" => $cityName,
//		);
//		$url = 'https://api.cdek.ru/city/getListByTerm/jsonPost.php';
//
//		$response = $this->_getRemoteData($data,$url,true);
//
//		if(is_array($response) && isset($response['geonames']) && is_array($response['geonames'])) {
//			foreach ($response['geonames'] as $item) {
//				if (mb_strtoupper($item["cityName"]) == mb_strtoupper($cityName)) {
//					$this->result = $item["id"];
//					return true;
//				}
//			}
//		}
//		$this->error = "Город назначения не найден";
//		return false;
		$data = array(
			"cityName" => $cityName,
			"country" => "Россия"
		...
		$url = 'https://integration.cdek.ru/v1/location/cities/json';

		$response = $this->_getRemoteData($data,$url,false);
		if(is_array($response) && count($response)) {
			...
			$this->result = $response[$out_key]['cityCode'];
			return true;
		}
	...
	public function getCityNameById ($cityId){
		...
		$response = $this->_getRemoteData($data,$url,false);
		if(is_array($response) && count($response)) {
			$this->result = $response[0]['cityName'];
			return true;
		}
		$this->error = "Город не найден";
		return false;
